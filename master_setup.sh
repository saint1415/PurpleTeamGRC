...
# Version: 3.0.4 - Fixed bash compatibility issues
# FIX: Improved POSIX compliance and error handling
################################################################################

set -e  # Exit on error
# Removed set -u to allow checking for unset variables
...
run_preflight_checks() {
    ...
    if [ -f /etc/os-release ]; then
        ...
        if [[ "$ID" == "kali" ]]; then
            ...
            # Check for Kali Purple - try multiple methods
            local has_purple=false
            local purple_tools_count=0
            
            # Method 1: Check for metapackages
            for pkg in "kali-purple" "kali-tools-defend" "kali-linux-purple"; do
                if dpkg -l | grep -q "^ii  $pkg " 2>/dev/null; then
                    print_success "Kali Purple detected ($pkg installed)"
                    has_purple=true
                    break
                fi
            done
            
            # Method 2: Check for individual Purple Team tool categories
            if [ "$has_purple" = "false" ]; then
                for tool in "kali-tools-detect" "kali-tools-respond" "kali-tools-protect"; do
                    if dpkg -l | grep -q "^ii  $tool " 2>/dev/null; then
                        purple_tools_count=$((purple_tools_count + 1))
                    fi
                done
                
                if [ "$purple_tools_count" -ge 2 ]; then
                    print_success "Kali Purple tools detected ($purple_tools_count tool categories installed)"
                    has_purple="true"
                fi
            fi
            
            if [ "$has_purple" = "false" ]; then
                print_warning "Kali Purple packages not detected"
                print_info "Defensive security tools may need installation"
                print_info "Try: apt install kali-tools-respond kali-tools-detect"
                warnings=$((warnings + 1))
            fi
        else
...
install_security_tools() {
    ...
    # Kali Purple menu and themes (optional)
    print_section "Installing Kali Purple Menu & Themes (optional)"
    
    # Check if menu/themes are already installed
    local menu_installed=$(dpkg -l | grep -q "^ii  kali-menu-purple " && echo "true" || echo "false")
    local theme_installed=$(dpkg -l | grep -q "^ii  kali-themes-purple " && echo "true" || echo "false")
    
    if [ "$menu_installed" = "false" ] || [ "$theme_installed" = "false" ]; then
        if confirm_action "Install Kali Purple menu and themes?"; then
            print_info "Installing Kali Purple UI components..."
            
            if [ "$menu_installed" = "false" ]; then
                DEBIAN_FRONTEND=noninteractive apt install -y kali-menu-purple 2>/dev/null && \
                    print_success "Kali Purple menu installed" || \
                    print_warning "Failed to install kali-menu-purple"
            fi
            
            if [ "$theme_installed" = "false" ]; then
                DEBIAN_FRONTEND=noninteractive apt install -y kali-themes-purple 2>/dev/null && \
                    print_success "Kali Purple themes installed" || \
                    print_warning "Failed to install kali-themes-purple"
            fi
        fi
    else
        print_success "Kali Purple menu and themes already installed"
    fi
